<?php
  ...
  if(isset($_POST['co_submit'])){
    if(isset($_POST['co_matricule']) && isset($_POST['co_password'])){
      if(strlen($_POST['co_matricule']) <= 255) {
        $matricule = htmlspecialchars($_POST['co_matricule']);
        $pass = sha1(htmlspecialchars($_POST['co_password']));
        $requser = $bdd->prepare("SELECT * FROM users WHERE matricule = ? AND password = ?");
        $requser->execute(array($matricule, $pass));
        $userexist = $requser->rowCount();
        if($userexist == 1){
          $userinfo = $requser->fetch();
          $_SESSION['id'] = $userinfo['id'];
          $_SESSION['matricule'] = $userinfo['matricule'];
          $_SESSION['division'] = $userinfo['division'];
          $_SESSION['grade'] = $userinfo['grade'];
          $_SESSION['numericalgrade'] = $userinfo['numericalgrade'];
          $_SESSION['pservice'] = $userinfo['p_service'];
                    ...
                    <div id="menu-container">
                        ...
                        <div id="menu-1" class="content home indexMenu" style="display: none;">
                            <div class="page-header">
                                <h2 class="page-title">Veuillez vous connecter</h2>
                            </div> <!-- /.page-header -->
                            <div class="content-inner">
                                <div class="row">
                                <form class="contact-form" method="POST">
                                    <input type="text" name="co_matricule" id="co_matricule" placeholder="Matricule (Ex: CCA-C17-GRADE-XXXXX | CCA-C17-DIVISION-GRADE-XXXXX)" />
                                    <input type="password" name="co_password" id="co_password" placeholder="Mot de passe" />
                                    <input type="submit" name="co_submit" id="co_submit" class="mainBtn" value="Connexion">
                                </form>
                                </div> <!-- /.row -->
                            </div> <!-- /.content-inner -->
                        </div> <!-- /.homepage -->
