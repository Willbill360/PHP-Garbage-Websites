<?php
  ...
  if(isset($_SESSION['id'])) {
    if(isset($_GET['id'])) {
        ...
        if(isset($_POST['lavage_cancel'])) {
            header('Location: dashboard?id='.$_SESSION['id']);
        } elseif (isset($_POST['lavage_submit'])) {
            if(isset($_POST['lavage_raison']) && isset($_POST['lavage_stage']) && !empty($_POST['lavage_stage']) && !empty($_POST['lavage_raison'])) {
                $raison = htmlspecialchars($_POST['lavage_raison']);
                $stage = htmlspecialchars($_POST['lavage_stage']);
                $date = date("Y-m-d");
                $insertlav = $bdd->prepare("INSERT INTO lavagescerveau(matricule, createur_matricule, date_creation, stage, raison) VALUES(?, ?, ?, ?, ?)");
                $insertlav->execute(array($curuser['matricule'], $_SESSION['matricule'], $date, $stage, $raison));
                header('Location: dashboard?id='.$_SESSION['id']);
            }
        }
?>
                    ...
                    <div id="menu-container">
                        ...
                        <div id="menu-1" class="content home indexMenu" style="display: none;">
                            <div class="page-header">
                                <h2 class="page-title">Lavage de cerveau pour le matricule <?= $curuser['matricule'] ?></h2>
                            </div> <!-- /.page-header -->
                            <div class="content-inner" style="padding-top: 0px;">
                                <div class="row">
                                <form class="contact-form" method="POST">
                                    <label>Stage</label><br/>
                                    <select name="lavage_stage">
                                        <option value="1">1</option>
                                        <option value="2">2</option>
                                        <option value="3">3</option>
                                    </select><br/><br/>
                                    <label>Raison</label>
                                    <textarea style="max-width:100%;width:100%;" name="lavage_raison" id="lavage_raison" placeholder="L'unité a témoin d'une chose dont elle n'était pas habilité."></textarea>
                                    <input type="submit" name="lavage_cancel" id="lavage_cancel" class="mainBtn" style="width: 49%;" value="Annuler">
                                    <input type="submit" name="lavage_submit" id="lavage_submit" class="mainBtn" style="width: 49%; float: right;" value="Soumettre">
                                </form>
                                </div> <!-- /.row -->
                            </div> <!-- /.content-inner -->
                        </div> <!-- /.homepage -->
