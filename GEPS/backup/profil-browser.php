...
$pseudo = $bdd->query('SELECT pseudo, id FROM membres ORDER BY id ASC');
if(isset($_GET['q']) AND !empty($_GET['q'])) {
   $q = htmlspecialchars($_GET['q']);
   $pseudo = $bdd->query('SELECT pseudo, id FROM membres WHERE pseudo LIKE "%'.$q.'%" ORDER BY id ASC');
   if($pseudo->rowCount() == 0) {
      $pseudo = $bdd->query('SELECT pseudo, id FROM membres WHERE CONCAT(pseudo, mail) LIKE "%'.$q.'%" ORDER BY id ASC');
   }
}
...
<html>
	<head>
		<title>G.E.P.S - Recherche de profil</title>
		<meta charset="utf-8" />
							...
							<div align="center" class="inner">
								...
								<div class="12u$">
								   ...
								   <?php while($a = $pseudo->fetch()) { ?>
								      <li><a class="button" href="profil-viewer?id=<?php echo $a['id']; ?>"><?= $a['pseudo'] ?></a></li>
								   <?php } ?>
								   </ul>
								</div>
								<?php } else { ?>
